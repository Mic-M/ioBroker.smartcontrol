...
class Trigger {
    ...
    async _asyncSetTargetDevices_processZone(zoneName) {
        ...
        try {
            ...
            for (const lpTargetDeviceName of this.zoneTargetNames[zoneName]) {
                ...
                /**
                 * Function switchTargetDevice - we use format 'const xyz = ()=>' to have all variables available
                 */
                const switchTargetDevice = async ()=> {

                    try {
                        
                        if (!this.zoneTargetNames) return;
                        ...
                        if (counterDevicesProcessed == this.zoneTargetNames[zoneName].length) { 

                            /**
                             * All devices processed, so we continue here
                             */
                            if (!this.zoneTargetNames || !this.zoneNames) return;
                                ...
                                if (this.motionNotIfManual) {
                                    for (const lpTargetDevice of lAlreadyOnDevices) {
                                        if (this.adapter.x.motionTriggeredDevices.indexOf(lpTargetDevice) > -1) {
                                            ...
                                            this.zoneTargetNames[zoneName] = this.adapter.x.helper.arrayRemoveElementByValue(this.zoneTargetNames[zoneName], lpTargetDevice);
                                            lRemovedDevices.push(lpTargetDevice);
                                        }
                                    }
                                }
                                if (this.zoneTargetNames[zoneName].length < 1) {
                                    ...
                                    if (lRemovedDevices.length == 0) {
                                        ...
                                        logMotion = `Timer of ${this.motionDuration}s will be set to turn off once no more motion recognized. Limited to '${this.zoneTargetNames[zoneName].toString()}'. ${lRemovedDevices.toString()} were turned on manually before.`;
                                    }
                                }
                            }
                            ...
                            if (this.triggerIsToggle && onOff == 'off') {
                                ...
                                const zoneOffTimeLeft = this.adapter.x.helper.getTimeoutTimeLeft(this.adapter.x.timersZoneOff[zoneName]);
                                if (zoneOffTimeLeft > -1) {
                                    this.adapter.log.debug(`Trigger [${this.triggerName}] Zone '${zoneName}': Turn off 'always off' timer, since still running ${(zoneOffTimeLeft / 1000).toFixed(0)} seconds and toggle trigger triggered to turn off.`);
                                    clearTimeout(this.adapter.x.timersZoneOff[zoneName]);
                                    this.adapter.x.timersZoneOff[zoneName] = null;
                                }

                            } else {
                                if (this.triggerIsMotion && (this.zoneTargetNames[zoneName].length < 1)) {
                                    ...
                                    this.asyncSetZoneTimer_zoneOff(zoneName);
                                }
                            }

                            // Set "Zone is on" status
                            this.adapter.x.zonesIsOn[zoneName] = true;

                            /**
                             * JSON log for info.log.zoneActivations.json
                             */
                            let maxJsonLength = 0;
                            if (this.adapter.x.helper.isNumber(this.adapter.config.jsonZoneActivationMaxEntries)) {
                                maxJsonLength = parseInt(this.adapter.config.jsonZoneActivationMaxEntries);
                            }
                            if (maxJsonLength > 0) {
                                const jsonEntry = {
                                    ts: Date.now(),
                                    time: this.adapter.x.helper.dateToLocalIsoString(new Date()),
                                    zone: zoneName,
                                    trigger: this.triggerName,
                                    targetsAll: this.zoneTargetNames[zoneName].toString(),
                                    targetsAllDelays: JSON.stringify(lDelaySecs),
                                    targetsSet: lSwitchedDevices.toString(),
                                    targetsSkipped: lAlreadyOnDevices.toString(),
                                    motionTimer: (this.motionDuration) ? this.motionDuration : 0,
                                    alwaysOffTimer: 0, // being set below
                                };
                                const alwaysOff = parseInt(this.adapter.getOptionTableValue('tableZones', 'name', zoneName, 'offAfter'));
                                jsonEntry.alwaysOffTimer = (isNaN(alwaysOff)) ? 0 : alwaysOff;
                                this.adapter.x.zonesLog = this.adapter.x.zonesLog.slice(0, maxJsonLength - 1); // just keep first x entries
                                this.adapter.x.zonesLog.unshift(jsonEntry); // Add new entry as first element of array
                                this.adapter.setState('info.log.zoneActivations.json', { val: JSON.stringify(this.adapter.x.zonesLog), ack: true });
                            }

                            // Finalize
                            if (lSwitchedDevices.length > 0) {
                                if (lAlreadyOnDevices.length < 1) {
                                    this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] activated Zone '${zoneName}'. Turned ${onOff}: ${lSwitchedDevices.toString()}.`);
                                } else {
                                    this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] activated Zone '${zoneName}'. Turned ${onOff}: ${lSwitchedDevices.toString()}. Not turned ${onOff} (as already ${onOff}): '${lAlreadyOnDevices.toString()}'`);
                                }
                            } else {
                                if (lAlreadyOnDevices.length > 0) this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] activated Zone '${zoneName}'. However, device(s) '${lAlreadyOnDevices.toString()}' not turned ${onOff} as these are already ${onOff}.`);
                            }
                            if (lDelayedDevices.length > 0) this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] Following device(s) will be switched ${onOff} later, since a delay was set: ${lDelayedDevices.toString()}.`);
                            if (logMotion) this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] Zone '${zoneName}': ${logMotion}`);

                        }

                    } catch (error) {
                        this.adapter.x.helper.dumpError(`[switchTargetDevice]`, error);
                        return;
                    }
                ...
                /**
                 * let's call function switchTargetDevice() as defined above
                 */
                ...
                if (delay > 0) {
                    // set timeout
                    this.adapter.x.timersZoneDeviceOnDelay[zoneName+'_'+ lpTargetDeviceName] = setTimeout(switchTargetDevice, delay * 1000);
                } else {
                    // call right away
                    switchTargetDevice();
                }

                // NOTE:Don't add any code below here, since we handle all above and finally set switchTargetDevice().
    ...
    async asyncAreScheduleConditionsMet(conditionNames, allMustMeet, inverse = false) {
        ...
        try {
            ...
            if (this.adapter.x.helper.isLikeEmpty(conditionNames)) {
                this.adapter.log.debug(`Trigger [${this.triggerName}]  No extra condition(s) in schedule, so we return "true" to continue.`);
                return true;
            }

            let hitCount = 0;

            this.adapter.log.debug(`Trigger [${this.triggerName}]  Check if additional conditions are met (like Public Holiday) - Processing conditions '${JSON.stringify(conditionNames)}'`);
            ...
            for (const lpConditionName of conditionNames) {
                ...
                if (res.isComparator && res.result) {
                    this.adapter.log.debug(`Trigger [${this.triggerName}] Condition '${lpConditionName}' *met* (options val = '${lpConditionStateValue}', actual state val = '${lpStateVal}')`);
                    hitCount++;
                    ...
                    // No hit, so go out!
                    this.adapter.log.debug(`Trigger [${this.triggerName}] Condition '${lpConditionName}' *not* met (options val = '${lpConditionStateValue}', actual state val = '${lpStateVal}')`);
                    notMetConditionNames.push(lpConditionName);
                    if (allMustMeet) { break; } else { continue; }
                }
            ...
            if (result) {
                this.adapter.log.debug(`Trigger [${this.triggerName}] additional conditions check - final result: matching`);
                return true;
            } else {
                if (!inverse) this.adapter.log.debug(`Trigger [${this.triggerName}] additional conditions check - final result: *not* matching conditions: ${notMetConditionNames.toString()}`);
                if (inverse) this.adapter.log.debug(`Trigger [${this.triggerName}] additional conditions check - final result: *not* matching conditions`); // to avoid empty 'notMetConditionNames'
                return false;
            }
    ...
    async _asyncSetZoneTimer_doOnTimeout(zoneName, timerTitle, targets, secTimer) {
        ...
        try {
            ...
            const switchOff = await this.asyncAreScheduleConditionsMet(neverOffConditionNames, allMustMeet, true);
            if (!switchOff) {
