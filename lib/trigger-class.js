 ...
 *    {boolean} mimicMotion              For trigger devices linked/associated with a motion sensor (tableTriggerMotion -> motionLinkedTrigger)
 *    {string}  mimicMotionTriggerName   Trigger name of device linked to motion sensor
 *    {boolean} mimicMotionIgnoreMotionOn Ignore motion if motion sensor is set to true
 *    ---Time Trigger---
...
class Trigger {
    ...
    _init() {
        ...
        try {
            ...
            this.mimicMotion = false;          // set accordingly in main.js
            this.mimicMotionTriggerName = undefined;  // set accordingly in main.js
            this.mimicMotionIgnoreMotionOn = false; // set to true below, if tableTriggerMotion.ignoreMotionOn is true
            ...
            switch (result.tableId) {
                ...
                case 'tableTriggerMotion':
                    ...
                    this.mimicMotionIgnoreMotionOn = result.row.ignoreMotionOn;
    ...
    async _asyncSetTargetDevices_processZone(zoneName) {
        ...
        try {
            ...
            } else if (!this.mimicMotion && this.triggerIsMotion) {
            ...
            /**
             * Ignore motion sensor triggered motion=true, if mimicMotionIgnoreMotionOn set to true
             */
            const timeLeft = this.adapter.x.helper.getTimeoutTimeLeft(this.adapter.x.timersMimicMotionTrigger[zoneName]);
            if (!this.mimicMotion && this.triggerStateVal && this.triggerIsMotion && this.mimicMotionIgnoreMotionOn) {
                if (timeLeft > -1) {
                    this.adapter.log.debug(`Trigger [${this.triggerName}] - Zone ${zoneName}: 'Mimic motion' is active, so we take over`);
                    // timer will be canceled below
                } else {
                    this.adapter.log.info(`Trigger [${this.triggerName}] - Zone ${zoneName}: No execution due to activated zone option 'ignore motion on'.`);
                    return;
                }
            }

            // Clear "linked devices" (mimic motion) timer for motion trigger
            if (this.triggerIsMotion && (timeLeft > -1)) {
                clearTimeout(this.adapter.x.timersMimicMotionTrigger[zoneName]);
                this.adapter.x.timersMimicMotionTrigger[zoneName] = null;
                this.adapter.log.debug(`Clearing timer 'Linked device to motion', which was still running for ${(timeLeft / 1000).toFixed(0)} seconds.`);
            }
                ...
                async function switchTargetDeviceAsync(that) {
                    ...
                    try {
                        ...
                        if (counterDevicesProcessed == that.zoneTargetNames[zoneName].length) { 
                             ...
                             */
                            let logMotion = '';
                            if (!that.mimicMotion && that.triggerIsMotion && that.motionDuration && that.motionDuration > 0) {
                            ...
                            if (that.triggerIsToggle && onOff == 'off') {
                                ...
                                if (that.triggerIsMotion && (that.zoneTargetNames[zoneName].length < 1)) {
                                    ...
                                    const zoneOffSecs = parseInt(that.adapter.getOptionTableValue('tableZones', 'name', zoneName, 'offAfter'));
                                    that.asyncSetZoneTimer_zoneOff('timersZoneOff', zoneName, zoneOffSecs);
                                }
                            }

                            /**
                             * If "mimic motion": set off timer for zone
                             */
                            if (that.mimicMotion) {
                                const zoneOffSecs = parseInt(that.adapter.getOptionTableValue('tableTriggerMotion', 'name', that.triggerName, 'motionLinkedDuration'));
                                that.asyncSetZoneTimer_zoneOff('timersMimicMotionTrigger', zoneName, zoneOffSecs);                                
                            }
                            ...
                            that.adapter.x.helper.setJsonLog('zoneActivations', {
                                ...
                                time: that.adapter.x.helper.dateToLocalIsoString(new Date()),
                                zone: zoneName,
                                trigger: (!that.mimicMotion) ? that.triggerName : that.mimicMotionTriggerName,
     ...
     * @param {string} timerObjectKey - e.g. 'timersZoneOff' or 'timersMimicMotionTrigger'
     * @param {string} zoneName     - Zone Name
     * @param {number} zoneOffAfter - Switch zone off after this number of seconds
     */
    async asyncSetZoneTimer_zoneOff(timerObjectKey, zoneName, zoneOffAfter) {

        try {

            if (!zoneOffAfter || zoneOffAfter < 1) {
            ...
            const timeLeft = this.adapter.x.helper.getTimeoutTimeLeft(this.adapter.x[timerObjectKey][zoneName]);
            if (timeLeft > -1) {
                clearTimeout(this.adapter.x[timerObjectKey][zoneName]);
                this.adapter.x[timerObjectKey][zoneName] = null;
            }

            // Set timer
            this.adapter.x[timerObjectKey][zoneName] = setTimeout(() => {
                this._asyncSetZoneTimer_doOnTimeout(zoneName, timerObjectKey, targets, zoneOffAfter);
            }, zoneOffAfter * 1000);

            // Some log
            if (timeLeft > -1) {
                this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] 'zoneOff' ${zoneOffAfter}s timer still running ${(timeLeft / 1000).toFixed(0)}s. We stop it and set new to ${zoneOffAfter}s.`);
            } else {
                this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] 'zoneOff' ${zoneOffAfter}s timer for zone '${zoneName}' initially started.`);
            }
    ...
    async _asyncSetZoneTimer_doOnTimeout(zoneName, timerTitle, targets, secTimer) {
        ...
        try {
            ...
            // Next, cancel all timers to ensure that another timer is not running.
            const timerObjIds = ['timersZoneOff', 'timersMimicMotionTrigger'];
            for (const lpTimerObjId of timerObjIds) {
                const lpTimeLeft = this.adapter.x.helper.getTimeoutTimeLeft(this.adapter.x[lpTimerObjId][zoneName]);
                if (this.triggerIsMotion && (lpTimeLeft > -1)) {
                    clearTimeout(this.adapter.x[lpTimerObjId][zoneName]);
                    this.adapter.x[lpTimerObjId][zoneName] = null;
                    this.adapter.log.debug(`Trigger [${this.triggerName}] Zone '${zoneName}': Clearing timer '${lpTimerObjId}', which was still running for ${(lpTimeLeft / 1000).toFixed(0)} seconds.`);
                }
            }
       

            /* ================== START -- RESOLVING ISSUE #45 (https://github.com/Mic-M/ioBroker.smartcontrol/issues/45) */

            /**
             * Verify if there are other motion sensors in the same zone. 
             * If yes and if a motion detection of one of these is still true: do not switch off zone.
             */
            // Get all motion sensor trigger names
            const allMotionNames = [];
            for (const lpMotionRow of this.adapter.config.tableTriggerMotion) {
                if (lpMotionRow.active) {
                    allMotionNames.push(lpMotionRow.name);
                }
            }
            // Get all other motion triggers assigned to zone
            const confZoneOtherTriggersMotion = [];
            const confZoneTriggersAll = this.adapter.getOptionTableValue('tableZones', 'name', zoneName, 'triggers');
            for (const lpTriggerName of confZoneTriggersAll) {
                if (allMotionNames.includes(lpTriggerName)) {
                    if (lpTriggerName !== this.triggerName) {
                        // we have an additional motion sensor trigger
                        confZoneOtherTriggersMotion.push(lpTriggerName);
                    }
                }
            }
            
            // Now check if motion state of any other motion timer is still true
            if (confZoneOtherTriggersMotion.length > 0) {
                for (const lpName of confZoneOtherTriggersMotion) {
                    const lpMotionObjId = this.adapter.getOptionTableValue('tableTriggerMotion', 'name', lpName, 'stateId');
                    const lpMotionObj = await this.adapter.x.helper.asyncGetForeignStateValue(lpMotionObjId);
                    if (lpMotionObj == true) {
                        // We have another motion sensor in same zone with motion=true, so go out.
                        this.adapter.x.helper.logExtendedInfo(`Trigger [${this.triggerName}] Zone '${zoneName}': Timer '${timerTitle}' ended, but another motion sensor '${lpName}' state value assigned to this zone is still true, so we will not switch off zone.' `);
                        return; // go out
                    }
                }
            }

            /* ================== END -- RESOLVING ISSUE #45 (https://github.com/Mic-M/ioBroker.smartcontrol/issues/45) */
