...
class Trigger {
    ...
    async _asyncSetTargetDevices_processZone(zoneName) {
        ...
        const targetsOverwrite = this.adapter.getOptionTableValue('tableZones', 'name', zoneName, 'targetsOverwrite'); // like: {'Hallway.Light':'new val' 'Hallway.Radio':'Radio XYZ'}
        ...
        if (lSwitchedDevices.length > 0) {
            ...
            this.adapter.x.helper.logExtendedInfo(`Trigger '${this.triggerName}' activated Zone '${zoneName}'. However, devices '${lAlreadyOnDevices.toString()}' not turned ${onOff} as these are already ${onOff}.`);
            if (logMotion) this.adapter.x.helper.logExtendedInfo(`Trigger '${this.triggerName}', Zone '${zoneName}': ${logMotion}`);
        }
    }
