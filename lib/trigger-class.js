...
class Trigger {
        ...
        try {
            ...
            for (const lpZoneName of this.zoneNames) {
                ...
                if (!zoneOnSecs || zoneOnSecs < 1) {
                    this.adapter.log.debug(`Zone '${lpZoneName}' No 'On after x sec' time is set, so we do not set such delay (timer).`);
    ...
    /**
     * Set all target devices: Process zone
        ...
        try {
            ...
            for (const lpTargetDeviceName of this.zoneTargetNames[zoneName]) {
                ...
                if (targetsOverwrite && targetsOverwrite[lpTargetDeviceName]) {
                    ...
                    if (targetsOverwrite[lpTargetDeviceName].delay!==undefined && this.adapter.x.helper.isNumber(targetsOverwrite[lpTargetDeviceName].delay)) {
                        overwriteDelay = targetsOverwrite[lpTargetDeviceName].delay;
                    }
                }

                // Timer (delay) to switch device on
                if (overwriteDelay > 0) lDelayedDevices.push(lpTargetDeviceName);
                ...
                this.adapter.x.timersZoneDeviceOnDelay[zoneName+'_'+ lpTargetDeviceName] = setTimeout(async () => {
                    ...
                    if (doNotCheckTarget || (!doNotCheckTarget && (currentStateVal != lpTargetVal))) {
                        ...
                        if (overwriteDelay == 0) lSwitchedDevices.push(lpTargetDeviceName);
                        ...
                        if (overwriteDelay == 0) lAlreadyOnDevices.push(lpTargetDeviceName);
                    }
            ...
            if (this.triggerIsToggle && onOff == 'off') {
                ...
                if (zoneOffTimeLeft > -1) {
                    this.adapter.x.helper.adapter.log.debug(`Zone '${zoneName}': Turn off 'always off' timer, since still running ${(zoneOffTimeLeft / 1000).toFixed(0)} seconds and toggle trigger triggered to turn off.`);
            ...
            if (lDelayedDevices.length > 0) this.adapter.x.helper.logExtendedInfo(`Following device(s) will be switched ${onOff} later, since a delay was set: ${lDelayedDevices.toString()}.`);
    ...
    async _asyncSetZoneTimer_doOnTimeout(zoneName, timerTitle, targets, secTimer) {
        ...
        try {
            ...
            if (!switchOff) {
                this.adapter.x.helper.logExtendedInfo(`Zone '${zoneName}': Timer '${timerTitle}' ended, but 'Never switch off if...' condition(s) true --> Zone will not be switched off.`);
                return; // go out
            }
