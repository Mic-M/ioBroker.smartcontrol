    ...
    async getTriggerConfigParamAsync(triggerName) {
        ...
        try {
            ...
            const cP = {
                ...
                // {array}   zoneNames              All zone names that were triggered
                // {object}  zoneTargetNames        All target device names per zone, like {Hallway:['Hallway Light'], Bath:['Light', 'Radio Bath']}
                // {object}  zoneScheduleRows       Schedule table rows triggered per zone, like {Hallway:[*schedule table rows*], Bath:[*schedule table rows*]}
            ...
            /**
             * Get zone names, and zone-specific target names and schedule rows
            ...
            cP.zoneScheduleRows = {}; // Schedule table rows triggered per zone, like {Hallway:[*schedule table rows*], Bath:[*schedule table rows*]}
            for (const lpRowZones of this._adapter.config.tableZones) {
                // Loop next if item if table "Zones" not containing any trigger name in column triggers
                ...
                // Check if zone is used in tableSchedules, and if schedule conditions are met
                /*let isHit = false;*/
                cP.zoneScheduleRows[lpRowZones.name] = [];  // Initiate new object item {'ZONE NAME' = []}
                for (const lpRow of this._adapter.config.tableSchedules) {
                    
                    let checkNext = true; // used since break command would break loop above
                    if (checkNext && lpRow.active && (lpRowZones.name == lpRow.name) ) {
                        ...
                        if (doContinue) doContinue = this.scheduleIsTimeStampWithinPeriod(tsCurrent, lpRow.start, lpRow.end, lpRow.name);
                
                        // Next, check if current day is within the mon-sun options as set in schedules tables.
                        if (doContinue) doContinue = this.scheduleIsWeekdayMatching(lpRow, tsCurrent);
                        ...
                        // All checks done.
                        if (doContinue) {
                            this._adapter.log.debug(`Execution table row for zone '${lpRow.name}' is meeting conditions`);
                            /* isHit = true; */
                            cP.zoneNames.push(lpRowZones.name); // set matching zone name
                            cP.zoneScheduleRows[lpRowZones.name].push(lpRow); // Add schedules row
                            checkNext = false; // used since break command would break loop above
                        } else {
                            this._adapter.log.debug(`Execution table row for zone '${lpRow.name}' is NOT meeting conditions`);
                        }                            

                    }
                    /*
                    if (isHit) {
                        this.logExtendedInfo(`Trigger '${cP.triggerName}' activated zone '${lpRowZones.name}' and we have a matching a schedule.`);
                    } else {
                        this.logExtendedInfo(`Trigger '${cP.triggerName}' activated zone '${lpRowZones.name}', but it is not matching a current schedule, so no execution.`);
            ...
            if (this.isLikeEmpty(cP.zoneNames) || this.isLikeEmpty(cP.zoneTargetNames) || this.isLikeEmpty(cP.zoneScheduleRows)) {
    ...
    async asyncVerifyConfig(functionConfigArray) {
        ...
        try {
            ...
            for (const functionConfigObj of functionConfigArray) {
                ...
                const lpConfigTableSettings = this._adapter.config[functionConfigObj.tableId]; 

                // Active status of each row; We simply push true or false in loop, same index as table rows (lpConfigTableSettings)
                ...
                for (let i = 0; i < lpConfigTableSettings.length; i++) {
                    ...
                    for (const lpCheckKey of lpCriteriaToCheck) {
                        ...
                        // lpFcnConfigCheckObj = like {id: 'stateId', type:'statePath', deactivateIfError:true }
                        ...
                        let lpConfigString = lpConfigTableSettings[i][lpFcnConfigCheckObj.id];
                        ...
                        if (lpFcnConfigCheckObj.type == 'statePath') {
                            ...
                            if (!this.isStateIdValid(lpConfigString)) {
                                ...
                                if (lpFcnConfigCheckObj.deactivateIfError) {
                                    lpConfigTableSettings[i].active = false;
                            ...
                            lpConfigTableSettings[i][lpFcnConfigCheckObj.id] = lpConfigString;
                            

                        // +++++++ VALIDATION TYPE: stateValue -- a value to be set to a certain state +++++++
                        } else if (lpFcnConfigCheckObj.type == 'stateValue') {

                            const lpStatePath       = lpConfigTableSettings[i][lpFcnConfigCheckObj.stateValueStatePath];
                        ...
                        // +++++++ VALIDATION TYPE: a number +++++++
                            ...
                            const lpNumberToCheck = lpConfigTableSettings[i][lpFcnConfigCheckObj.id];
                            ...
                            if(! this.isNumber(lpNumberToCheck) ) {
                                ...
                                if (lpFcnConfigCheckObj.deactivateIfError) lpConfigTableSettings[i].active = false;
                                errors++; 
                                continue;
                            // check for lower limit, if 'numberLowerLimit' set in functionConfigObj object
                                ...
                                if(parseInt(lpNumberToCheck) < lpFcnConfigCheckObj.numberLowerLimit) {
                                    this._adapter.log.warn('[Config Table \'' + functionConfigObj.tableName + '\'] Number in field "' + lpFcnConfigCheckObj.id + '" is smaller than ' + lpFcnConfigCheckObj.numberLowerLimit + ', this does not make sense at all!');
                                    if (lpFcnConfigCheckObj.deactivateIfError) {
                                        lpConfigTableSettings[i].active = false;
                                    }
                                    errors++; 
                            ...
                            // check for upper limit, if 'numberUpperLimit' set in functionConfigObj object
                                ...
                                if(parseInt(lpNumberToCheck) < lpFcnConfigCheckObj.numberUpperLimit) {
                                    this._adapter.log.warn('[Config Table \'' + functionConfigObj.tableName + '\'] Number in field "' + lpFcnConfigCheckObj.id + '" is greater than ' + lpFcnConfigCheckObj.numberUpperLimit  + ', this does not make sense at all!');
                                    if (lpFcnConfigCheckObj.deactivateIfError) lpConfigTableSettings[i].active = false;
                                    errors++;
                                    continue;
                                }
                        ...
                        // +++++++ VALIDATION TYPE: name (which can also be a drop down with multiple values) +++++++
                            ...
                            let lpToCheck = lpConfigTableSettings[i][lpFcnConfigCheckObj.id];
                            ...
                            lpConfigTableSettings[i][lpFcnConfigCheckObj.id] = lpToCheck;


                        // +++++++ VALIDATION TYPE: time and timeCron +++++++
                        // -- We test for both "time" and "timeCron".
                        } else if (lpFcnConfigCheckObj.type.substring(0, 4) == 'time') {
                            const lpToCheck = lpConfigTableSettings[i][lpFcnConfigCheckObj.id];
                        ...
                        // -- for tableZones -> targetsOverwrite
                            ...
                            for (const lpProperty in targetsOverwriteObject) {
                                ...
                                if (!lpStatePath) throw(`Unexpected Error - Unable to retrieve state path by name '${name}' from tableTargetDevices.`);
                ...
                for (const lpRow of lpConfigTableSettings) {
                ...
                this._adapter.config[functionConfigObj.tableId] = lpConfigTableSettings;
        ...
        try {
            ...
            if (statePath.match(/^smartcontrol.\d+./)) {
                if (stateObject.ack == false) {
                    this._adapter.log.debug(`[isAckPassing()] Adapter instance state (${this._adapter.namespace}) identified -> passing since ack:false`);
                    return true;
                } else {
                    this._adapter.log.debug(`[isAckPassing()] Adapter instance state (${this._adapter.namespace}) identified -> NOT passing since ack:true (we require ack:false)`);
                    return false;
                }
            }
            ...
            if (isUserState) {
                ...
                if (!this._adapter.config.triggerStatesAck || this._adapter.config.triggerStatesAck == 'false') {
                    if (stateObject.ack == false) {
                        this._adapter.log.debug(`[isAckPassing()] User state identified -> passing per adapter config '${this._adapter.config.triggerStatesAck}'`);
                        return true;
                    } else {
                        this._adapter.log.debug(`[isAckPassing()] User state identified -> NOT passing per adapter config '${this._adapter.config.triggerStatesAck}'`);
                        return false;
                    }
                ...
                if (stateObject.ack == true) {
                    this._adapter.log.debug(`[isAckPassing()] "Real" adapter instance state identified -> passing since ack:true`);
                    return true;
                } else {
                    this._adapter.log.debug(`[isAckPassing()] "Real" adapter instance state identified -> NOT passing since ack:false`);
                    return false;
                }
            }
