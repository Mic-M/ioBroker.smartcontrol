...
class Library {
     ...
     * !                                smartcontrol.0.options.Zones.Flur EG.Lichter.Wandlicht 1.active
                    ...
                    if (zoneRow.active && zoneRow.name == lpZone) {
                        ...
                        for (const lpTriggerName of zoneRow.triggers) {
                            if (this._timers.motion[lpTriggerName]) {
                                ...
                                if (timeLeft > -1) {
                                    this._adapter.log.debug(`Zone '${zoneRow}': Timer for ${lpTriggerName} still running ${(timeLeft/1000).toFixed(0)} seconds but a motion from ${zoneRow} detected, which is associated to same zone. Therefore, we stop timer and set new.`);
    ...
    async asyncSwitchTargetDevices(cP) {
        ...
        try {
            ...
            const lSwitchedDevices = [];      // for log only
            ...
            if (cP.triggerIsMotion) {
                // Set Motion Timer
                this.asyncSetZonesTimer(cP, true);
                logMotion = `Setting timer of ${cP.motionDuration}s to turn off.`;
            } 
            // Also, set timer for offAfter if set in Table Zones.
            ...
            if (lSwitchedDevices.length > 0) {
                if (lAlreadyOnDevices.length < 1) {
                    ...
                    this.logExtendedInfo(`Trigger '${cP.triggerName}', Zone '${cP.zoneNames.toString()}' activated. '${lSwitchedDevices.toString()}' not turned ${onOff} (as already ${onOff}): '${lAlreadyOnDevices.toString()}'. ${logMotion}`);
                }
                // Set offAfter timer
                    ...
                    for (const lpCheckKey of lpCriteriaToCheck) {
                        ...
                        // +++++++ VALIDATION TYPE: name (which can also be a drop down with multiple values) +++++++
                            ...
                            if(typeof lpToCheck == 'string' && lpCheckVal.removeForbidden) {
                                const forbidden = this.forbiddenCharsInStr(lpToCheck, this._globals.forbiddenStatePaths);
                                if (forbidden) {
