...
class Library {
    ...
    /**
     * This is called once an adapter state changes like 'smartcontrol.0.Options.Areas.Hallway.active'
     * 
    ...
    async handleStateChangeOptionsActive(statePath, stateObject) {
        ...
        try {
            ...
            if (stateObject.ack) return; // ack must be false

            // {name:'Hallway', index:2, table:'tableAreas', field:'active', row:{.....} }
            ...
            if (optionObj.row[optionObj.field] == stateObject.val) {
                this._adapter.log.debug(`State '${statePath}' changed, but is equal to current option, so no action at this point.`);
                return;
            }
            ...
            const what = (stateObject.val) ? 'activated' : 'de-activated';
            this._adapter.log.info(`'${optionObj.table}': row '${optionObj.name}' ${what}. Adapter instance will be restarted to put the change into effect.`);
     ...
     * 
     * @param {string} statePath  like 'smartcontrol.0.Options.Areas.Hallway.active'
     * 
    ...
    async asyncGetOptionForOptionStatePath(statePath) {
        ...
        try {
            ...
            const stOptionTable = statePathSplit[3]; // Like 'Areas'
            const stOptionName = statePathSplit[4]; // Like 'Hallway'
            const stOptionField = statePathSplit[5]; // Like 'active'
            let cName; // The final Option name from state 'smartcontrol.0.Options.Areas.Hallway.name'
            try {
        ...
        try {
            ...
            if (allAdapterStates != undefined) {
                for (const lpState of allAdapterStates) {
                    const statePath = lpState._id; // like: 'smartcontrol.0.Options.Areas.Hallway.name'
                ...
                this._schedules[lpRow.name] = this._Schedule.scheduleJob(cron, async () => {
                    try {
                        ...
                        if(cP && cP.triggerName) {
                            ...
                            if(!doContinue) {
                                this.logInfo(`Schedule ${cP.triggerName} (time: ${cP.triggerTime}) triggered, but condition(s) not met.`);
                                return;
                            }
    ...
    async getTriggerConfigParam(triggerType, trigger, triggerStateValSet=undefined) {
        ...
        try {
            ...
            for (const lpScheduleRow of cP.scheduleRows) {
                const targetDeviceNames = this.getOptionTableValue('tableAreas', 'name', lpScheduleRow.name, 'targets');
                if (this.isLikeEmpty(targetDeviceNames)) {
                    this._adapter.log.error(`No target devices for room ${lpScheduleRow.name} found.`);
                    return null;
                }
     ...
     * @param {ioBroker.State|null|undefined} stateObject    State object
     */
    async asyncTargetDeviceTriggered(statePath, stateObject) {
        ...
        try {
            ...
            // Verify if a motion sensor was the trigger and a brightness is set and if state bri is smaller than defined threshold.
            if( cP.triggerIsMotion
                ...
                if ( this.isLikeEmpty(briStateValCurrent) || !this.isNumber(briStateValCurrent) || parseInt(briStateValCurrent) < 0) {
                    ...
                    this._adapter.log.debug(`Trigger [${cP.triggerName}] activated but current brightness of ${briStateValCurrent} is >= than the setting threshold of ${cP.motionBriThreshold}. Therefore, we disregard the motion.`);
                    return;
            ...
            this._adapter.log.error(`[asyncTargetDeviceTriggered] ${error}`);
            return false;
        }
        ...
        async function verifyIfScheduleConditionsTrue (thisRef) {
            ...
            try {
                ...
                for (const lpRowTableSchedule of cP.scheduleRows) {
                    ...
                    // All checks done.
                    if (doContinue) {
                        thisRef._adapter.log.debug(`Schedule for room '${lpRowTableSchedule.name}' is meeting conditions`);
                        isHit = true;
                        break;
                    } else {
                        thisRef._adapter.log.debug(`Schedule for room '${lpRowTableSchedule.name}' is NOT meeting conditions`);
                    }
                }
    ...
    async getSchedulesOfTrigger(trigger, what) {
        ...
        try {
            ...
            for (const lpName of triggerNames) {
                for (const lpRowTableAreas of this._adapter.config.tableAreas) {
                    if (lpRowTableAreas.active && lpRowTableAreas.triggers && Array.isArray(lpRowTableAreas.triggers)) {
                        /** @type {array} */
                        const triggers = lpRowTableAreas.triggers;
                        if ( (triggers.includes(lpName) ) ) {
                            ...
                            for (const lpRowTableSchedules of this._adapter.config.tableSchedules) {
                                if (lpRowTableSchedules.active && (lpRowTableAreas.name == lpRowTableSchedules.name) ) {
    ...
    async asyncVerifyConfig(toCheckArray) {
        ...
        try {
                ...
                for (let i = 0; i < lpConfigTableSettings.length; i++) {
                    ...
                    for (const lpCheckKey of lpCriteriaToCheck) {
                        ...
                        if (lpCheckVal.type == 'statePath') {
                            ...
                            if ( this.isLikeEmpty(lpConfigString.trim()) && lpCheckVal.optional) {
                            ...
                            if (!this.isStateIdValid(lpConfigString)) {
                                ...
                                this._adapter.log.warn(`[Config Table '${toCheck.tableName}'] State '${lpConfigString}' is not valid.`);
            ...
            const uniqueCheckObjects = [
                ...
                { name:'Room/Area table: Names',                    column:'name',      rows:this._adapter.config.tableAreas },
