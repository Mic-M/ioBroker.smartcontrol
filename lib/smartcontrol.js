...
class Library {
    ...
    async setTimerMotionSensor(cP) {
        ...
        try {
            ...
            this._adapter.log.error(`[setTimerMotionSensor()] ${error.message}, stack: ${error.stack}`);
        }
    }
    ...
    async getConfigParametersForTriggerState(triggerStatePath) {
        ...
        try {
            ...
            this._adapter.log.error(`[getConfigParametersForTriggerState()] ${error.message}, stack: ${error.stack}`);
            return false;
        }
    ...
    async asyncSwitchTargetDevices(scheduleRows, triggerStatePath) {
        ...
        try {
            ...
            if (! await verifyIfScheduleConditionsTrue(this, scheduleRows)) return false;
        ...
        async function verifyIfScheduleConditionsTrue (thisRef, scheduleRows) {
            ...
            try {
                ...
                if (isHit) {
                    thisRef.logInfo(`Trigger is matching a schedule.`);
                    return true;
                } else {
                    thisRef.logInfo(`Trigger is not matching a schedule, so no further action at this point.`);
                    return false;
                }
            
                
            } catch (error) {
                thisRef._adapter.log.error(`[verifyIfScheduleConditionsTrue()] ${error.message}, stack: ${error.stack}`);
                return false;
            }
    ...
    async asyncAreScheduleConditionsMet(conditionNames, allMustMeet, inverse=false) {
        ...
        try {
            ...
            this._adapter.log.error(`[asyncAreScheduleConditionsMet()] ${error.message}, stack: ${error.stack}`);
            return false;
        }
    ...
    async asyncCreateStates(statesToCreate, isForeign=false) {
        ...
        try {
            ...
            for (const lpStateToCreate of statesToCreate) {
                ...
                // Create new state. set(Foreign)ObjectNotExistsAsync takes care if state does not exist.
                if (isForeign) {
            ...
            this._adapter.log.error(`[asyncCreateForeignStates()] ${error.message}, stack: ${error.stack}`);
            return true;
        }
