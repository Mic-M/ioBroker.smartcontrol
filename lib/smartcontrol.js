    ...
    async asyncSwitchTargetDevices(cP) {
        ...
        try {
            ...
            for (const lpTargetDeviceName of cP.targetDeviceNames) {
                ...
                // let toggleHow = null; // null means we do not have toggle, false means "switch off", true means "switch on"
                if (cP.triggerIsToggle) {
                    ...
                    if (onVal == oldStateVal) {
                        // toggleHow = false; // Device is on, so doToggle should be false (turn off)
                        onOff = 'off';
                    } else {
                        onOff = 'on';
                        // toggleHow = true; // Device is off, so doToggle should be false (turn on)
                    }
                }
