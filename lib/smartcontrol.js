...
class Library {
    ...
    async getTriggerConfigParam(triggerType, trigger, triggerStateValSet=undefined) {
        ...
        try {
            ...
            if (triggerType == 'statePath') {
                ...
                cP.targetOff = false; // set later to true, if set in options table
    ...
    async asyncTriggerActivated(statePath, stateObject) {
        ...
        try {
            ...
            if (cP.triggerType == 'statePath' && !cP.triggerIsMotion && (typeof cP.triggerStateVal == 'string') ) {
                ...
                if (matchComparator != null) {
                    ...
                    const num = parseInt(matchComparator[2]);
                    if (operand == '>=' && parseInt(cP.triggerStateValSet) >= num ) {
                        comparatorSuccess = true;
                    } else if (operand == '<=' && parseInt(cP.triggerStateValSet) <= num ) {
                        comparatorSuccess = true;
                    } else if (operand == '>' && parseInt(cP.triggerStateValSet) > num ) {
                        comparatorSuccess = true;
                    } else if (operand == '<' && parseInt(cP.triggerStateValSet) < num ) {
                        ...
                        return; // We go out since we have a comparator, but not matching
                    }
                }
            }
            // Compare state value with config            
            if (cP.triggerType == 'statePath' && !comparatorSuccess) {
                if (cP.triggerStateVal != cP.triggerStateValSet) return; // Go out since no match
            }
        ...
        try {
            ...
            if (lSwitchedDevices.length > 0) {
                if (lAlreadyOnDevices.length < 1) {
                    this.logExtendedInfo(`Trigger '${cP.triggerName}' activated for '${cP.zoneNames.toString()}'. Turned ${onOff} '${lSwitchedDevices.toString()}'. ${logMotion}`);
                } else {
                    this.logExtendedInfo(`Trigger '${cP.triggerName}' activated for '${cP.zoneNames.toString()}'. Turned ${onOff}: '${lSwitchedDevices.toString()}'; Not turned ${onOff} (as already ${onOff}): '${lAlreadyOnDevices.toString()}'. ${logMotion}`);
                }
            } else {
                this.logExtendedInfo(`Trigger '${cP.triggerName}' activated for '${cP.zoneNames.toString()}'. However, devices '${lAlreadyOnDevices.toString()}' not turned ${onOff} as these are already ${onOff}. ${logMotion}`);
            }
     ...
     *         3)  If it exists, add execution table row to result array
     * 
     * @param {string} trigger            Either state path of device that was triggered, or timer trigger name
     * @param {string} what               'statePath' for a State Path, or 'timeName' for name of a time trigger
     * @return {Promise<array>}           Array: rows from Execution table, which are associated with the trigger.
     */
        ...
        try {
            ...
            if(!this.isLikeEmpty(finalScheduleRows)) {
                ...
                if (triggerNames.length == 1) {
                    this._adapter.log.debug(`Execution rows for '${triggerNames[0]}' successfully found.`);
                } else {
                ...
                if (triggerNames.length == 0) {
                    ...
                    this._adapter.log.error(`Trigger activation '${triggerNames[0]}' failed: no rows found in Execution table.`);
                } else {
                    this._adapter.log.error(`Trigger activation ${JSON.stringify(triggerNames)} failed: no rows found in Execution table.`);
                }
                return [];
            }
