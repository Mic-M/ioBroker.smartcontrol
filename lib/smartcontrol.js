 ...
 * ! Schedule table renamed to Execution in admin on 2020-07-11 -- may be confusing for variable names.
 * 
...
class Library {
     ...
     *  @param {boolean} [motionTriggered=false]  - // ! Describe
     * 
     */
    async asyncSetZonesTimer(cP, motionTriggered=false) {

        try {

            // Go out if no duration is set
            if (motionTriggered && (!cP.motionDuration || cP.motionDuration < 0 )) return;

            // Check if a timer for the same zone is already running and kill it.
            for (const lpZone of cP.zoneNames) {
                for (const zoneRow of this._adapter.config.tableZones) {
                    if (zoneRow.active && zoneRow.name == lpZone) {
                        // Now check all triggers
                        for (const lpTriggerName of zoneRow.triggers) {
                            if (this._timers.motion[lpTriggerName]) {
                                const timeLeft = this.getTimeoutTimeLeft(this._timers.motion[lpTriggerName]);
                                if (timeLeft > -1) {
                                    this._adapter.log.debug(`Zone '${zoneRow.name}': Timer for ${lpTriggerName} still running ${(timeLeft/1000).toFixed(0)} seconds but a motion from ${lpTriggerName} detected, which is associated to same zone. Therefore, we stop timer and set new.`);
            ...
            if (motionTriggered) {
                ...
                this._timers.motion[cP.triggerName] = setTimeout(async ()=> {
                    await this.asyncSetZonesTimer_doOnTimeout(cP, targets, cP.motionDuration);
                }, cP.motionDuration * 1000);
            } else {
                
                // ========== Set offAfter timer ==========

                // we can have multiple zone names which were triggered
                for (const lpZone of cP.zoneNames) {
                    let zoneRow;
                    for (const lpRow of this._adapter.config.tableZones) {
                        if (lpRow.name == lpZone) {
                            zoneRow = lpRow;
                            break;
                        } 
                    }
                    if (!zoneRow) {
                        throw(`Zone ${lpZone} not found in Zones table while executing offAfter.`);
                    } else {

                        // Check if we have offAfter seconds
                        const sec = parseInt(zoneRow.offAfter);
                        if (!sec || sec < 1) {
                            break;
                        } else {
                            // Set offAfter timer
                            this._timers.zoneOff[cP.lpZone] = setTimeout(async ()=> {
                                await this.asyncSetZonesTimer_doOnTimeout(cP, targets, sec);
                            }, sec * 1000);
                            this.logExtendedInfo(`Timer for "always off after ${zoneRow.offAfter} seconds" started for Zone '${zoneRow.name}'.`);
                        }
            ...
            this.dumpError(`[asyncSetZonesTimer]`, error);
            return;
        }
     ...
     */
    async asyncSetZonesTimer_doOnTimeout(cP, targets, secTimer) {
        ...
        try {
            ...
            for (let i = 0; i < targets.statePaths.length; i++) {
                ...
                if (targetOffStateValCurrent == null) {
                    this._adapter.log.error(`Trigger '${cP.triggerName}' - Timeout of '${secTimer}' seconds reached, but could not get current state value of '${lpStatePath}', so unable to turn device off.`);
                    continue;
                }
                if (lpStateValToSet == targetOffStateValCurrent) {
                    this.logExtendedInfo(`Trigger '${cP.triggerName}' - Timeout of '${secTimer}' seconds reached, but target device '${lpTargetName}' is already turned off, so no action at this point.`);
                    continue;
                }
            ...
            if (lManualOnDevices.length > 0) {
                this.logExtendedInfo(`Trigger '${cP.triggerName}' - Timeout of '${secTimer}' seconds reached, but target device(s) '${lManualOnDevices.toString()}' manually turned on while timer was running, so do not turn off.`);
            }
            if (lTargetDevicesSwitchedOff.length > 0) {
                this.logExtendedInfo(`Trigger '${cP.triggerName}', Zone '${cP.zoneNames.toString()}':  Turned off target device(s) '${cP.targetDeviceNames.toString()}' per ${secTimer}s timer.`);
            }                    


        } catch (error) {
            this.dumpError('[asyncSetZonesTimer_doOnTimeout]', error);
            return;
        }
     ...
     * @return {object|null}          - The return object. We return null in case of errors
     */
    getTriggerConfigParam(triggerName) {
        ...
        try {
            ...
            cP.scheduleRows = this.getTriggerExecutionSchedules(triggerName);
            if(cP.scheduleRows == null) return null; // error msg already thrown in calling function
    ...
    async asyncTriggerActivated(statePath, stateObject) {
        ...
        try {
            ...
            for (const lpTriggerName of triggerNames) {
                ...
                const cP = await this.getTriggerConfigParam(lpTriggerName);
                if (cP == null) throw(`No valid config parameters found for ${statePath}`);
                ...
                if (cP.triggerTableId == 'tableTriggerDevices' && (typeof cP.triggerStateVal == 'string') ) {
                    ...
                    if (matchComparator != null) {
                        ...
                        if (operand == '>=' && Number(stateObject.val) >= num ) {
                            ...
                            this._adapter.log.debug(`Trigger '${lpTriggerName}' activated, but not meeting conditions, therefore, we disregard the activation.`);
    ...
    async asyncSwitchTargetDevices(cP) {
        ...
        try {
            ...
            for (const lpTargetDeviceName of cP.targetDeviceNames) {
                ...
                const lpTargetVal   = this.getOptionTableValue('tableTargetDevices', 'name', lpTargetDeviceName, onOff+'Value');
                ...
                const oldStateVal = await this.asyncGetForeignStateValue(lpTargetState);
                if (oldStateVal == null) {
            ...
            let logMotion = '';
            const cP_motion = { ...cP }; // clone (copy) object. Needed since we alter cP.targetDeviceNames for motion timer
            if (cP.triggerIsMotion) {
                ...
                if (cP.motionNotIfManual) {
                    for (const lpTargetDevice of lAlreadyOnDevices) {
                        if ( this._motionTriggeredDevices.indexOf(lpTargetDevice) > -1 ) {
                            ...
                            cP_motion.targetDeviceNames = this.arrayRemoveElementByValue(cP_motion.targetDeviceNames, lpTargetDevice);
                ...
                // Set Motion Timer
                if (cP_motion.targetDeviceNames.length < 1) {
                    ...
                    if (lRemovedDevices.length == 0) {
                        ...
                        logMotion = `Setting timer of ${cP.motionDuration}s to turn off, limited to '${cP_motion.targetDeviceNames.toString()}'. ${lRemovedDevices.toString()}  turned on manually before.`;
                    }
                    // Now set timer 
                    this.asyncSetZonesTimer(cP_motion, true);
                }
            } 

            // Also, set "always off after" timer if set in Table Zones.
            if (cP.triggerIsToggle && onOff == 'off') {
                // Toggle is activated and zone is turned off, so kill/delete the timer.
                for (const lpZone of cP.zoneNames) {
                    for (const zoneRow of this._adapter.config.tableZones) {
                        if (zoneRow.active && zoneRow.name == lpZone) {
                            this._adapter.log.debug(`Zone '${zoneRow}': Delete 'always off' timer, as toggle trigger triggered to turn off.`);
                            clearTimeout(this._timers.zoneOff[cP.lpZone]);
                            this._timers.zoneOff[cP.lpZone] = null;
                        }
                    }
                }
            } else {
                // Set the "always off after" timer
                if (cP.triggerIsMotion && (cP_motion.targetDeviceNames.length < 1) ) {
                    ...
                    // Set the "always off after" timer
                    this.asyncSetZonesTimer(cP, false);
                }
            }
            ...
            if (lSwitchedDevices.length > 0) {
                if (lAlreadyOnDevices.length < 1) {
                    this.logExtendedInfo(`Trigger '${cP.triggerName}' activated Zone '${cP.zoneNames.toString()}'. Turned ${onOff}: ${lSwitchedDevices.toString()}. ${logMotion}`);
                } else {
                    this.logExtendedInfo(`Trigger '${cP.triggerName}', Zone '${cP.zoneNames.toString()}' activated. '${lSwitchedDevices.toString()}' turned ${onOff}. Not turned ${onOff} (as already ${onOff}): '${lAlreadyOnDevices.toString()}'. ${logMotion}`);
                }
            } else {
                this.logExtendedInfo(`Trigger '${cP.triggerName}', Zone '${cP.zoneNames.toString()}' activated. However, devices '${lAlreadyOnDevices.toString()}' not turned ${onOff} as these are already ${onOff}. ${logMotion}`);
            }
        ...
        async function verifyIfScheduleConditionsTrue (thisRef) {
            ...
            try {
                ...
                for (const lpRowTableSchedule of cP.scheduleRows) {
                    ...
                    thisRef._adapter.log.debug(`Trigger ${cP.triggerName} is associated with following Execution table row: ${JSON.stringify(lpRowTableSchedule)}`);
                    ...
                    // All checks done.
                    if (doContinue) {
                        thisRef._adapter.log.debug(`Execution table row for zone '${lpRowTableSchedule.name}' is meeting conditions`);
                        isHit = true;
                        break;
                    } else {
                        thisRef._adapter.log.debug(`Execution table row for zone '${lpRowTableSchedule.name}' is NOT meeting conditions`);
                    }
                }
                ...
                if (isHit) {
                    ...
                    thisRef.logExtendedInfo(`Trigger '${cP.triggerName}' activated, but it is not matching a current schedule, so no execution.`);
                    return false;
                }
    ...
    /**
     * For an activated trigger, get the according rows of Execution table.
     * 
     * @param {string} triggerName - name of any trigger: motion, other, time
     * @return {array|null}         Array: rows from Execution table, which are associated with the trigger.
     */
    getTriggerExecutionSchedules(triggerName) {

        try {

            let triggerRow;
            for (const lpRow of this._adapter.config.tableTriggerMotion.concat(this._adapter.config.tableTriggerDevices, this._adapter.config.tableTriggerTimes)) {
                if (lpRow.active && lpRow.name == triggerName) {
                    triggerRow = lpRow;
                    break; // per asyncVerifyConfig, names are unique.
                }
            }

            if (this.isLikeEmpty(triggerRow)) {
                throw(`No row found in trigger tables for name '${triggerName}'.`);
            }

            // Check the table "Zones" for any rows containing this trigger name in column triggers
            const finalExecutionRows = [];
            for (const lpRowZones of this._adapter.config.tableZones) {
                if (lpRowZones.active && lpRowZones.triggers && Array.isArray(lpRowZones.triggers)) {
                    /** @type {array} */
                    const zoneTriggers = lpRowZones.triggers;
                    if ( (zoneTriggers.includes(triggerName) ) ) {
                        /**
                         * We need to go to the table Execution ("tableSchedules"). We use the zone name, in which we found the trigger.
                         * Since multiple schedules can be set for one zone, we get all rows containing the zone name into finalExecutionRows
                         */
                        for (const lpRowExecution of this._adapter.config.tableSchedules) {
                            if (lpRowExecution.active && (lpRowZones.name == lpRowExecution.name) ) {
                                finalExecutionRows.push(lpRowExecution);
                            }
                        }
                    }
                }
            ...
            if (finalExecutionRows.length > 0) {
                this._adapter.log.debug(`${finalExecutionRows.length} rows in Execution table for trigger '${triggerName}' successfully found.`);
                return finalExecutionRows;
            } else {
                ...
                throw (`Trigger '${triggerName}': activation failed - no active row for this trigger found in Execution table.`);
            }

        } catch (error) {
            this.dumpError('[getSchedulesOfTrigger()]', error);
            return null;
        }
    ...
    async asyncVerifyConfig(toCheckArray) {
        ...
        try {
                ...
                for (let i = 0; i < lpConfigTableSettings.length; i++) {
                    ...
                    for (const lpCheckKey of lpCriteriaToCheck) {
                        ...
                        // +++++++ VALIDATION TYPE: stateValue -- a value to be set to a certain state +++++++
                            ...
                            if (!lpStateObject) {
                                ...
                                if (lpStateType == 'boolean' || lpStateType == 'switch') {
                                    ...
                                    const isComparator = (/^(>=|<=|>|<)\s?(\d{1,})$/.test(lpConfigString.trim()));
                                    if (isComparator) {
                                        this._adapter.config[toCheck.tableId][i][lpCheckVal.id] = lpConfigString.trim();
                                    } else if (this.isNumber(lpConfigString)) {
                        ...
                        // +++++++ VALIDATION TYPE: name (which can also be a drop down with multiple values) +++++++
                            ...
                            if (lpCheckVal.removeForbidden) {
                                ...
                                for (let i = 0; i < checkForbiddenArray.length; i++) {
                                    ...
                                    if (forbidden) {
                                        ...
                                        this.logExtendedInfo(`[Config Table '${toCheck.tableName}'] Field value '${checkForbiddenArray[i]}' contains forbidden character(s) '${forbidden}', so we remove from string.`);
                                        if (Array.isArray(lpToCheck)) {
                            ...
                            if (this.isLikeEmpty(lpToCheck) &&  !lpCheckVal.optional) {
            ...
            if (errors == 0) {
                ...
                this._adapter.log.error(`[Adapter Config Verification Failed] ${errors} error(s) found --> Please check the log for details and then correct your configuration accordingly. You will not be able to use this adapter without fixing the issue(s).`);
                return false;
            }
            ...
            for (const lpRow of this._adapter.config.tableTriggerTimes) {
                if (lpRow.active) {
                    ...
                    this._schedules[lpRow.name] = this._Schedule.scheduleJob(cron, async () => {
                        ...
                        const cP = await this.getTriggerConfigParam(lpRow.name);
                        if(cP && cP.triggerName) {
                            ...
                            this.logExtendedInfo(`Execution table row ${cP.triggerName} (time: ${cP.triggerTime}) triggered.`);
                            this.asyncSwitchTargetDevices(cP);
                        }
    ...
    scheduleIsWeekdayMatching(row, ts) {
        ...
        for (const lpDay of days) {
            if (lpWeekdayGiven == null) {
                ...
                this._adapter.log.debug(`Current weekday '${lpWeekdayGiven}' *is* matching Execution table row for '${row.name}'.`);
                return true;
            }
        }        
        // There was no hit, so we return false.
        this._adapter.log.debug(`Current weekday '${lpWeekdayGiven}' is *not* matching Execution table row for '${row.name}'.`);
        return false;
    ...
    scheduleIsTimeStampWithinPeriod(tsGiven, start, end, ofWhat) {
        ...
        try {
            ...
            if (this.isTimeBetween(startFinal, endFinal, givenFinal)) {
                this._adapter.log.debug(`Current time '${givenFinal}' *is* within schedule times (start: '${startFinal}', end: '${endFinal}) of '${ofWhat}'.`);
                return true;                
            } else {
                this._adapter.log.debug(`Current time '${givenFinal}' is *not* within schedule times (start: '${startFinal}', end: '${endFinal}) of '${ofWhat}'.`);
                return false;                                
            }
