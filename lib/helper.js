    ...
    return {
        ...
        async asyncCreateStates(statesToCreate, isForeign=false) {
            ...
            try {
                ...
                for (const lpStateToCreate of statesToCreate) {
                    ...
                    if (isForeign && ! await this.asyncObjectExists(lpStateToCreate.statePath, true)) {
                    ...
                    } else if(!isForeign && ! await this.asyncObjectExists(lpStateToCreate.statePath, false)) {
        ...
        async asyncObjectExists (str, isForeign=false) {
            ...
            try {
                ...
                this.dumpError('[asyncObjectExists()]', error);
                return false;
            }
        ...
        */
         ...
         * @return {number}  The time remaining of the setTimeout() in milliseconds. Returns -1, if not or no longer running, never run, or object not even set.
         *                   To convert ms to seconds, simply use: (timeLeft / 1000).toFixed(0) - where timeLeft is the return value of the function.
         * 
         */
        ...
        /**
         * JSON Log to smartcontrol.x.info.log.<id>.json
         * @param {string} id         - e.g. 'zoneActivations' or 'switchedTargetDevices'
         * @param {object} jsonObj    - object to add, e.g. {zone:'xyz', targetDevice:'xyz'}
         * @return {boolean} - false if errors caught, otherwise true.
         */
        async setJsonLog(id, jsonObj) {

            try {

                let maxJsonLength = 0;
                if (adapter.x.helper.isNumber(adapter.config.jsonMaxEntries)) { 
                    maxJsonLength = parseInt(adapter.config.jsonMaxEntries);
                }
                if (maxJsonLength <= 0) return true;
                
                if(!this.isLikeEmpty(adapter.x.jsonLogs[id])) {
                    adapter.x.jsonLogs[id] = adapter.x.jsonLogs[id].slice(0, maxJsonLength - 1); // just keep first x entries
                    adapter.x.jsonLogs[id].unshift(jsonObj); // Add new entry as first element of array                    
                } else {
                    adapter.x.jsonLogs[id] = [jsonObj]; // add as array
                }
                adapter.setState(`info.log.${id}.json`, { val: JSON.stringify(adapter.x.jsonLogs[id]), ack: true });
                return true;

            } catch (error) {
                this.dumpError('[setJsonLog]', error);
                return false;
            }
         ...
         * @param {boolean} [returnMsg=false] - Optional: If true, it will not log an error, but return the msg as string
         * @return {void|string}
         */
        dumpError(msg, error=undefined, returnMsg=false) {
            ...
            if (!error) {
                ...
                if (typeof error === 'object') {
                    if (error.stack) {
                        finalMsg = `${msg} – ${error.stack}`;
                    } else if (error.message) {
                        finalMsg = `${msg} – ${error.message}`;
                    } else {
                        finalMsg = `${msg} – ${JSON.stringify(error)}`;
                    }
                } else if (typeof error === 'string') {
                    finalMsg = `${msg} – ${error}`;
                } else {
                    finalMsg = `[dumpError()] : wrong error argument: ${JSON.stringify(error)}`;
        ...
        /**
         * Converts string like 'val:'abc', delay:500' to a JSON object
         * If keys 'val' or 'delay' are not existing, return {val: query}
         * @param {string} query
         * @param {array} keys - array of strings like ['val', 'delay']
         * @param {boolean} [flat=false] 
         * @return {object|null} JSON object, or null if query is empty or errors
         */
        convertStrToJson(query, keys, flat=false) {

            if (this.isLikeEmpty(query)) return null;

            let keyFound = false;
            for (const lpKey of keys) {
                const regex = new RegExp(`'?"?${lpKey}'?"?:\\s?`, '');
                if (regex.test(query)) {
                    keyFound = true;
                    break;
                }
            ...
            if ( keyFound ) {
                ...
                const modifiedArray = [];
                for (const lpEntryStr of arrayOfKeyValues) {
                    const lpEntryArr = lpEntryStr.split(':');
                    const key = this.strCharsRemove(lpEntryArr[0], /['"`]/g, true)[0];
                    const val = this.strCharsRemove(lpEntryArr[1], /['"`]/g, true)[0];
                    const newEntryStr = `"${key}":"${val}"`;
                    modifiedArray.push(newEntryStr);
                }
                ...
                if (flat) {
                    ...
                    try {
                        return JSON.parse('{' + modifiedArray.toString() + '}');
                    } catch (error) {
                        adapter.log.error('[convertStrToJson())]: Could not convert given query to JSON object');
                        return null;
                    }
                }
            } else {
                const newVal = this.strCharsRemove(query, /['"`]/g, true)[0];
                if (flat) {
                    return newVal;
                } else {
                    return {val: newVal};
        ...
        /**
         * Validate URL
         * @param {string} url - given URL as string
         * @param {boolean} [clean=false] - removes white spaces if true
         * @param {boolean} [returnUrl=false] - if false: returns boolean, if true: url if successful, null if not
         * @return {boolean|string|null}
         */
        validateURL(url, clean=false, returnUrl=false) {
            if(clean) url = url.replace(/\s/g,''); // remove all white spaces
            const urlRegexStr = `^(https?)://([a-zA-Z0-9.-]+(:[a-zA-Z0-9.&%$-]+)*@)*((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]?)(.(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])){3}|([a-zA-Z0-9-]+\.)*[a-zA-Z0-9-]+.?([a-zA-Z]))(:[0-9]+)*(/($|[a-zA-Z0-9.,?'\\+&%$#=~_-]+))*$`;
            const regex = new RegExp(urlRegexStr);
            const isUrlValid = url.length < 2083 && regex.test(url);
            if (returnUrl) {
                if (isUrlValid) {
                    return url;
                } else {
